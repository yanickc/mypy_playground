...
def print_beer_song(items: Iterable[str]):
    for x in items:
        print(x)


def print_beer_song_time_to_sing(items: Sized):
    print(f"{len(items)} minutes to drink that song")


beers = BeerCrate(5)
print_beer_song_time_to_sing(beers)
print_beer_song(beers)

my_song = ['a', 'b', 'c']
print_beer_song_time_to_sing(my_song)
print_beer_song(my_song)
